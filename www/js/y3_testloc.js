...
var y3 = {
	...
	syncresult: function(){
		...
		if (f > 0) {
			...
			{
            	navigator.notification.confirm(_('clicca Scarica per scaricare i nuovi files'), y3.triggerDownload, f+_(' Aggiornamenti disponibili'), [_('Annulla'),_('Scarica')]);
            }
            else
            {
		...
		else
		{
			...
			if (navigator.notification != undefined)
			{
				navigator.notification.alert(_('Non ci sono file da aggiornare'), y3.triggerDownload, _('Nessun aggiornamento'), _('Chiudi'));	
			}
			else
			{
    ...
    showDownloadResult: function(errcode){
        ...
        case 0:
            ...
            message = _('Tutti i file sono sincronizzati.');
            break;
        case 1:
            title = _('Download annullato');
            message = _("Operazione interrotta. Per ripetere l'aggiornamento premi nuovamente sync.");
            break;
        case 2:
            title = _('Errore 002 durante il download');
            message = _("Si è verificato un errore durante lo scaricamento. Riprova in un secondo momento.");
            break;
        case 3:
            title = _('Errore 003');
            message = _("Si è verificato un errore durante lo scaricamento dei Gruppi. Riprova in un secondo momento.");
            break;
        }
